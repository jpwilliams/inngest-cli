...
async function getBinaryUrl(): Promise<URL> {
  ...
  const { arch, platform } = await getArchPlatform();
 ...
 */
async function getArchPlatform() {
...
/**
 * Takes a `req` and pipes it through unpacking and saving to the given
 * `location`.
 *
 * Will create the target location if it doesn't already exist.
 *
...
(async () => {
  ...
  try {
    /**
     * We could perform some of these calls in parallel, but handling
     * conditionally aborting the request is messy, so we'll take a small perf hit
     * here so we don't have to worry about it.
     */
